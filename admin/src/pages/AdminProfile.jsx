...
const AdminProfile = ({ token, adminData }) => {
        ...
        const fetchAdminDetails = async () => {
            try {
                const response = await axios.get(
                    `${backendUrl}/api/admin/profile/${adminData.adminID}`,
                    {
                        headers: {
                ...
                if (response.data.success) {
                    ...
                    setPreviewUrl(adminData.profilePicture ? `${backendUrl}/${adminData.profilePicture}` : null);
                } else {
  ...
  <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#232526] to-[#4682A9] p-4">
    <div className="backdrop-blur-lg bg-white/10 border border-white/20 shadow-xl rounded-3xl max-w-lg w-full p-8 md:p-12 flex flex-col items-center">
      <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-8 tracking-tight drop-shadow-lg">Admin Profile</h2>
      <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="inline-block rounded-full bg-[#4682A9] text-white px-4 py-1 text-lg font-bold tracking-wide shadow">Admin</span>
                    </div>
        <div className="flex flex-col gap-1">
